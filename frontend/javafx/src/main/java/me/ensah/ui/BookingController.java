...
public class BookingController {
    @FXML
    private Label scheduleRouteLabel;
    @FXML
    private Label scheduleMetaLabel;
    @FXML
    ...
    public void initialize() {
        ...
        paymentMethodCombo.setItems(FXCollections.observableArrayList("Credit Card", "Debit Card", "PayPal"));
        ...
        String route = String.format("%s  |  %s → %s", s.getTrainName(), s.getDepartureStationName(),
                s.getArrivalStationName());
    ...
    private VBox buildPassengerCard(Passenger p, int index) {
        ...
        javafx.scene.control.Spinner<Integer> ageSpinner = new javafx.scene.control.Spinner<>(0, 120,
                Math.max(0, p.getAge()), 1);
        ageSpinner.setEditable(true);
        ageSpinner.setPrefWidth(120);
        ageSpinner.valueProperty().addListener((obs, ov, nv) -> {
            p.setAge(nv == null ? 0 : nv);
        });
    ...
    public void confirmBooking() {
        ...
        if (method == null || cardNum.isBlank() || expiry.isBlank() || cvv.isBlank()) {
            warn("Please fill in all payment details.");
            return;
        }
        ...
        CompletableFuture.runAsync(() -> {
            try {
                // 1. Create Booking
                BookingResponse bookingRes = bookingService.createBooking(req);

                // 2. Process Payment
                BigDecimal totalAmount = BigDecimal.valueOf(s.getPrice()).multiply(BigDecimal.valueOf(payload.size()));
                PaymentRequest payReq = new PaymentRequest(
                        bookingRes.getBookingId(),
                        method,
                        totalAmount,
                        cardNum,
                        expiry,
                ...
                PaymentResponse payRes = bookingService.processPayment(payReq);

                Platform.runLater(() -> {
                    confirmBtn.setDisable(false);
                    confirmBtn.setText("Pay & Confirm");
                    success("Payment Successful! Booking Confirmed. Ref: " + bookingRes.getReferenceCode());
                    goHome();
                });
    ...
    private void goLogin() {
        javafx.stage.Stage stage = currentStage();
        if (stage != null)
            Navigation.goTo(stage, "/fxml/LoginView.fxml");
        else
            Platform.runLater(this::goLogin);
    }
